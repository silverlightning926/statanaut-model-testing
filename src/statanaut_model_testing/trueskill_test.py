...
ratings = {team: trueskill.Rating() for team in teams["key"]}
...
for i, match in two_team_matches.iterrows():
    ...
    if red_win:
        new_red, new_blue = trueskill.rate([red, blue], ranks=[0, 1])
    else:
        new_blue, new_red = trueskill.rate([blue, red], ranks=[0, 1])
